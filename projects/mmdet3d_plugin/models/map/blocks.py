...
@POSITIONAL_ENCODING.register_module()
class KeyPoint3DEncoder(BaseModule):
    def __init__(
        self,
        embed_dims: int = 256,
        num_sample: int = 6,
        ...
        super(KeyPoint3DEncoder, self).__init__()
        self.embed_dims = embed_dims
        ...
        self.embed_points = embedding_layer(self.coords_dim)
        self.embed_instance = embedding_layer(self.input_dims)

    def forward(self, anchor: torch.Tensor):
        bs, num_anchor, _ = anchor.shape

        anchor_reshape = anchor.view(bs, num_anchor, self.num_sample, self.coords_dim)
        anchor_reshape = anchor_reshape.view(bs, -1, self.coords_dim)

        points_embed = anchor_reshape[..., :2]
        points_embed = self.embed_points(points_embed)
        points_embed = points_embed.view(bs, num_anchor * self.num_sample, -1)

        instance_embed = self.embed_instance(anchor)

        return instance_embed, points_embed
...
class SparsePoint3DRefinementModule(BaseModule):
    def __init__(
        ...
        with_line_key_points = False,
    ):
        super(SparsePoint3DRefinementModule, self).__init__()
        self.embed_dims = embed_dims
        self.num_sample = num_sample
        self.with_line_key_points = with_line_key_points
        if self.with_line_key_points:
...
class SparsePoint3DKeyPointsGenerator(BaseModule): 
    def __init__(
        ...
        fix_height: Tuple = (0,),
        ground_height: int = 0,
        with_points_embed: bool = False,
        with_anchor_embed: bool = False,
    ):
        super(SparsePoint3DKeyPointsGenerator, self).__init__()
        self.embed_dims = embed_dims
        self.num_sample = num_sample
        self.num_learnable_pts = num_learnable_pts
        self.with_points_embed = with_points_embed
        self.with_anchor_embed = with_anchor_embed

        if self.with_points_embed:
            self.num_pts = len(fix_height) * num_learnable_pts
        else:
    ...
    def forward(
        ...
        ### learn offset
        if self.with_anchor_embed:
            # use point embed
            if self.with_points_embed:
                instance_feature_w_embed = instance_feature.repeat(1, self.num_sample, 1) + anchor_embed
                offset = (
                    self.learnable_fc(instance_feature_w_embed)
                    .reshape(bs, num_anchor, self.num_sample, len(self.fix_height), self.num_learnable_pts, 2)
                )
            else:
                offset = (
                    self.learnable_fc(instance_feature + anchor_embed)
                    .reshape(bs, num_anchor, self.num_sample, len(self.fix_height), self.num_learnable_pts, 2)
                )
        else:
            offset = (
